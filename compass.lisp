...
(define-override (compass minimum-height) ()
  (+ (if north (q+:minimum-height north) 0)
     (max (if center (q+:minimum-height center) 0)
          (if east (q+:minimum-height east) 0)
          (if west (q+:minimum-height west) 0))
     (if south (q+:minimum-height south) 0)))

(define-override (compass minimum-width) ()
  (max (if north (q+:minimum-width north) 0)
       (if south (q+:minimum-width south) 0)
       (+ (if east (q+:minimum-width east) 0)
          (if center (q+:minimum-width center) 0)
          (if west (q+:minimum-width west) 0))))

(define-override (compass maximum-height) ()
  (+ (if north (q+:maximum-height north) 0)
     (min (if center (q+:maximum-height center) most-positive-fixnum)
          (if east (q+:maximum-height east) most-positive-fixnum)
          (if west (q+:maximum-height west) most-positive-fixnum))
     (if south (q+:maximum-height south) 0)))

(define-override (compass maximum-width) ()
  (min (if north (q+:maximum-width north) most-positive-fixnum)
       (if south (q+:maximum-width south) most-positive-fixnum)
       (+ (if east (q+:maximum-width east) 0)
          (if center (q+:maximum-width center) 0)
          (if west (q+:maximum-width west) 0)
          (if (or east center west) 0 most-positive-fixnum))))
