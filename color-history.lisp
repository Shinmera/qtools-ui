...
(defmethod (setf value) (value (color-history color-history))
  (unless (= (q+:rgba (direct-value (widget 0 color-history)))
             (q+:rgba value))
...
(define-override (color-history-swatch mouse-release-event) (ev)
  (qtenumcase (q+:button ev)
    ((q+:qt.left-button)
     (rotate-colors (parent color-history-swatch)
                    (- (widget-position color-history-swatch (parent color-history-swatch)))))
    ((q+:qt.right-button)
